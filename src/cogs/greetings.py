...
class Greetings(commands.Cog):
  ...
  async def on_member_join(self, member: discord.Member):
    ...
    profile = Editor(profile_img).resize(
      (1195 - 695, 958 - 459)).circle_image()

    member_name = member.display_name if len(
      member.display_name) <= 9 else member.display_name[:10] + '..'
    username = f'@{member_name}'

    joined_at = datetime.fromtimestamp(
      member.joined_at.timestamp()).strftime("%H:%M")

    is_center = len(member_name) <= 9

    background_img.paste(profile, (695, 459))
    background_img.text((630, 1092) if not is_center else (945, 1092),
                        username,
                        lato_font,
                        color='#FFFFFF',
                        align=f'{"center" if is_center else "left"}')
    ...
    await member.add_roles(member.guild.get_role(1141721487338901586))
    await member.send(f'Selamat datang di *JURNALISTIK TCR*!')
  ...
  async def on_member_remove(self, member: discord.Member):
    ...
    profile_img = await load_image_async(str(member.display_avatar.url))
    profile = Editor(profile_img).resize(
      (1162 - 701, 702 - 229)).circle_image()

    member_name = member.display_name if len(
      member.display_name) <= 11 else member.display_name[:12] + '..'
    username = f'@{member_name}'

    joined_at = datetime.fromtimestamp(
      member.joined_at.timestamp()).strftime("%d-%m-%Y")
    leave_at = datetime.fromtimestamp(
      datetime.now().timestamp()).strftime("%d-%m-%Y")
    ...
    background_img.text((1890 // 2, 769),
                        username,
                        jeepers_font,
                        color='#525252',
                        align='center')
